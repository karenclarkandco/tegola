...
func NewRouter(a *atlas.Atlas) *httptreemux.TreeMux {
	...
	group.UsingContext().Handler(observability.InstrumentAPIHandler(http.MethodGet, "/:map_name/:z/:x/:y", o, HeadersHandler(GZipHandler(TileCacheHandler(a, hMapLayerZXY)))))
	group.UsingContext().Handler(observability.InstrumentAPIHandler(http.MethodGet, "/:map_name/:layer_name/:z/:x/:y", o, HeadersHandler(GZipHandler(TileCacheHandler(a, hMapLayerZXY)))))
